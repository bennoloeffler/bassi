...
class TestPermissionEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_permission_with_empty_tool_name(self, permission_manager, config_service):
    ...
    @pytest.mark.asyncio
    async def test_permission_with_none_tool_input(self, permission_manager, config_service):
        ...
        result = await permission_manager.can_use_tool_callback("Bash", None, None)
        assert isinstance(result, PermissionResultAllow)
    
    @pytest.mark.asyncio
    async def test_permission_timeout(self, permission_manager, config_service):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(future, timeout=0.1)  # Short timeout for test
    
    @pytest.mark.asyncio
    async def test_permission_without_websocket(self, permission_manager, config_service):
        ...
        result = await permission_manager.can_use_tool_callback("Bash", {}, None)
        assert isinstance(result, PermissionResultDeny)
    
    @pytest.mark.asyncio
    async def test_concurrent_permission_requests(self, permission_manager, config_service):
    ...
    @pytest.mark.asyncio
    async def test_invalid_permission_scope(self, permission_manager, config_service):
    ...
    @pytest.mark.asyncio
    async def test_permission_persistence_across_sessions(self, permission_manager, config_service):
    ...
    @pytest.mark.asyncio
    async def test_zero_count_one_time_permission(self, permission_manager, config_service):
