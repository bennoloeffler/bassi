...
class TestConcurrentMessageSaving:
    ...
    async def test_concurrent_message_saving(self, tmp_path):
        ...
        """
        workspace = SessionWorkspace("test-concurrent", base_path=tmp_path, create=True)
        ...
        assert workspace.metadata["message_count"] == 10, "Message count should be exactly 10"
        
        # Verify all messages in history
        history_content = (workspace.physical_path / "history.md").read_text()
        for i in range(10):
            assert f"Message {i}" in history_content, f"Message {i} should be in history"
        
        # Verify no duplicate counts
        message_lines = [line for line in history_content.split("\n") if "Message" in line and "Message" in line.split()[0]]
        # Count actual message content lines
        message_count_in_file = history_content.count("Message ")
        assert message_count_in_file == 10, f"Should have exactly 10 messages, found {message_count_in_file}"
        
        # Verify metadata file is consistent
        import json
        metadata_file = workspace.physical_path / "session.json"
        metadata_data = json.loads(metadata_file.read_text())
        assert metadata_data["message_count"] == 10, "Metadata file should match workspace metadata"
    
    @pytest.mark.asyncio
    async def test_concurrent_assistant_messages(self, tmp_path):
        """Test concurrent assistant message saving."""
        workspace = SessionWorkspace("test-concurrent-assistant", base_path=tmp_path, create=True)
        ...
        # Verify all messages saved
        assert workspace.metadata["message_count"] == 6  # 1 user + 5 assistant
...
class TestConcurrentFileUploads:
    ...
    async def test_concurrent_file_uploads(self, tmp_path):
        ...
        """
        workspace = SessionWorkspace("test-concurrent-upload", base_path=tmp_path, create=True)
...
class TestSessionIndexRaceConditions:
    ...
    async def test_concurrent_session_index_updates(self, tmp_path):
        ...
        assert len(session_ids) == len(set(session_ids)), "No duplicate sessions"
...
class TestWorkspaceMetadataRaceConditions:
    ...
    async def test_concurrent_metadata_updates(self, tmp_path):
        ...
        - Verify no data loss
        """
        workspace = SessionWorkspace("test-metadata-race", base_path=tmp_path, create=True)
