...
def test_load_session_with_messages(page, live_server):
    ...
    print(
        f"🔍 Browser console logs (showing all {len(console_messages)} messages):"
    )
    ...
    assert len(loaded_messages) == len(
    ...
    assert any(
        "first message" in m["content"] for m in user_messages
    ), "Original user message should be present after session switch"

    print(
        "✅ TEST PASSED: Messages loaded successfully after session switch!"
    )
...
def test_session_sidebar_shows_message_count(page, live_server):
    ...
    assert (
        "message" in message_count_text.lower()
