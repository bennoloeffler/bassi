...
def _send_message(page, text):
    ...
    page.wait_for_function(
        f"(count) => document.querySelectorAll('.user-message').length > count",
...
def test_agent_context_restored_after_session_switch(page, live_server):
    ...
    assert messages_in_session_1 >= 1, "Session 1 should have at least 1 user message"
    ...
    assert messages_after_switch >= 1, (
    ...
    print("")
    print("Note: Full verification requires real agent (not mock) to test memory.")
...
def test_agent_context_separate_between_sessions(page, live_server):
    ...
    assert "alice" in response1.lower(), f"Session 1 should remember Alice, got: {response1}"
    assert "bob" not in response1.lower(), f"Session 1 should NOT know about Bob from Session 2"
    ...
    assert "bob" in response2.lower(), f"Session 2 should remember Bob, got: {response2}"
    assert "alice" not in response2.lower(), f"Session 2 should NOT know about Alice from Session 1"
