...
def test_websocket_invalid_message_format(page, live_server):
    ...
    assert response["status"] in ["ok", "timeout"], "Server should respond after invalid message"
...
def test_websocket_send_during_session_deletion(page, live_server):
    ...
    delete_btn = page.query_selector(f"button[data-session-id='{session_id}'][title*='Delete']")
    if delete_btn:
        delete_btn.click()
...
def test_websocket_multiple_rapid_messages(page, live_server):
    ...
    messages = page.query_selector_all(".user-message, .chat-message, [data-message]")
    message_count = len(messages)
    assert message_count > 0, f"At least one message should appear, found {message_count}"

    print(f"✅ Rapid messages handled - {message_count} messages found, UI stable")
...
def test_file_upload_invalid_file_type(page, live_server):
    ...
    if file_input:
        ...
        try:
            ...
            print("✅ Invalid file upload handled (accepted or rejected gracefully)")
