"""
Chat Workspace Manager

Manages chat-specific file storage and organization following the
chat workspace architecture.

Physical path: chats/{chat_id}/
Display name: Stored in metadata (mutable)
Symlink: chats-human-readable/{timestamp}__{name}__{short-id} -> ../chats/{chat_id}

NOTE: This is the renamed version of SessionWorkspace.
...
class ChatWorkspace:
    """
    Manages a single chat's workspace.
    ...
    - Manage chat metadata (display_name, state, etc.)

    NOTE: Renamed from SessionWorkspace. "Chat" is clearer than "Session"
    ...
    def __init__(
        ...
        Args:
            chat_id: Unique chat identifier (UUID format)
            base_path: Base directory for all chats
            create: If True, create directory structure
        """
        self.chat_id = chat_id
        self.base_path = base_path
        self.physical_path = base_path / chat_id
        self._upload_lock = asyncio.Lock()

        # Symlink directory for human-readable chat names
    ...
    def session_id(self) -> str:
        """Backward compatibility: session_id -> chat_id."""
        return self.chat_id
    ...
    def _load_or_create_metadata(self) -> dict:
        ...
        # Support both old (session.json) and new (chat.json) names
        metadata_path_new = self.physical_path / "chat.json"
        metadata_path_old = self.physical_path / "session.json"

        # Check for new name first, then old name
        if metadata_path_new.exists():
            with open(metadata_path_new, "r", encoding="utf-8") as f:
                return json.load(f)
        elif metadata_path_old.exists():
            # Migrate from old name to new name
    ...
    def _save_metadata(self, metadata: Optional[dict] = None) -> None:
        """Save metadata to chat.json."""
        if metadata is None:
            metadata = self.metadata

        metadata_path = self.physical_path / "chat.json"
    ...
    def _create_initial_symlink(self) -> None:
        ...
        Format: {iso-datetime}__new-chat__{short-id}
        Example: 2025-11-08T14-30-45-123456__new-chat__a1b2c3d4
        """
        ...
        short_id = self.chat_id[:8]
        symlink_name = f"{timestamp}__new-chat__{short_id}"

        # Create symlink with correct relative target
        symlink_path = self.SYMLINK_DIR / symlink_name
        target = Path("..") / self.base_path.name / self.chat_id
    ...
    def delete(self) -> None:
        ...
        WARNING: This permanently deletes all chat data.
        """
        import shutil
    ...
    def update_display_name(self, new_name: str) -> None:
        """
        Update chat display name and symlink.
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "chat_id": self.chat_id,
            "session_id": self.chat_id,  # Backward compatibility
    ...
    @classmethod
    def load(
        cls, chat_id: str, base_path: Path = Path("chats")
    ) -> "ChatWorkspace":
        """
        Load existing chat workspace.

        Args:
            chat_id: Chat ID to load
            base_path: Base directory for chats

        Returns:
            ChatWorkspace instance

        Raises:
            FileNotFoundError: If chat doesn't exist
        """
        workspace_path = base_path / chat_id

        if not workspace_path.exists():
            raise FileNotFoundError(f"Chat not found: {chat_id}")

        return cls(chat_id, base_path, create=False)

    @classmethod
    def exists(cls, chat_id: str, base_path: Path = Path("chats")) -> bool:
        """Check if chat workspace exists."""
        workspace_path = base_path / chat_id
        # Support both old (session.json) and new (chat.json) names
        return workspace_path.exists() and (
            (workspace_path / "chat.json").exists()
            or (workspace_path / "session.json").exists()
        )
    ...
    def get_workspace_context(self) -> str:
        ...
        context_lines.extend(
            [
                ...
                "",
                f"1. **Reading user files:** Use `chats/{self.chat_id}/DATA_FROM_USER/filename`",
                f"2. **Saving results:** Use `chats/{self.chat_id}/RESULTS_FROM_AGENT/filename`",
                f"3. **Saving scripts:** Use `chats/{self.chat_id}/SCRIPTS_FROM_AGENT/filename`",
                f"4. **Saving data:** Use `chats/{self.chat_id}/DATA_FROM_AGENT/filename`",
                "",
