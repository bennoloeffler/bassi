"""
Try 06 (SDK): Clean HTML generator + persisted user sessions.

Design:
- Each websocket connection gets its own ClaudeSDKClient with sticky agent_session.
- Per-user history saved under chats/try06_sdk/<user>.json (agent_session + messages).
- UI: dropdown of known chats, refresh/load/new chat, markdown-ish rendering, user color.
- HTTP served via ThreadingHTTPServer; WS via websockets 15 API.
"""
...
STORAGE_DIR = Path("chats/try06_sdk")
...
def save_user(user: str, agent_session: str, messages: list[dict]) -> None:
    _user_path(user).write_text(
        json.dumps(
            {"agent_session": agent_session, "messages": messages}, indent=2
        )
    )
...
def build_html(ws_port: int, users: list[str]) -> str:
    users_json = json.dumps(users)
    return f"""<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Try 06 SDK — Persisted Sessions</title>
  <style>
...
<body>
  <h3>Try 06 SDK — Persisted Sessions</h3>
...
async def main(http_port: int = 9335, ws_port: int = 9336) -> None:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            if self.path == "/users":
                ...
                body = build_html(ws_port, list_users()).encode()
    ...
    print(
        f"[try06-sdk] HTTP http://127.0.0.1:{http_port} | WS ws://127.0.0.1:{ws_port}/ws"
    )
    try:
